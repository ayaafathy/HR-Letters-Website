...
<?php
	...
	if ( null==$id ) {
			header("Location: EMPC.php");
	}
	if(isset($_POST["Save"]))
	{
	$text = mysqli_real_escape_string($dbConn,$_POST["Comment"]);
	$sql="UPDATE  `letter_req`SET `Empcomments`='$text' WHERE Req_ID=$id ";
					 ...
					 <h5 style='text-align: center;'>WARNING </h5></div>
							 <div class='modal-body'>
								 <p>You Error Occured, try again later</p>
									 ...
									 <div class='modal-footer'>
										 <button type='button' id='back' class='btn btn-danger' data-dismiss='modal' onclick='myFunction();';>Back</button>
									 </div>
							 </div>
					 </div>
			 </div>";
